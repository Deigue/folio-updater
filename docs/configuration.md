...
Each account's tax type will try to infer from the
`<BROKER>-<TYPE>` naming convention: `IBKR-TFSA` is a TFSA, `WS-PERSONAL` is
non-registered, `QT-RRSP` is an RRSP. Recognised types are `NON_REGISTERED`, `MARGIN`,
`CORPORATE`, `TFSA`, `RRSP`, `RRIF`, `RESP`, `FHSA` and `LIRA`;

`NONREG`, `NREG`, `PERSONAL`, `CASH` and `TAXABLE` all resolve to non-registered.

`accounts.map` is an escape hatch for names the convention cannot resolve, such as a raw
broker account number. Both a shorthand and a full mapping are accepted:

```yaml
accounts:
  naming_convention: true          # set false to require an explicit map entry
  map:
    U1234567: NON_REGISTERED       # shorthand: just the type
    QT-TFSA:                       # full form
      type: TFSA
      amount_includes_fees: true
  defaults:
    amount_includes_fees: auto     # auto | true | false
```

`amount_includes_fees` says whether a broker's `Amount` on a trade already contains the
commission. QuestTrade's does; Interactive Brokers' and Wealthsimple's do not. The
default of `auto` reconciles each account's own trades against `Price * Units` and takes
the majority, which is why the key is almost never needed. Set it explicitly only if the
engine reports `AMBIGUOUS_FEE_CONVENTION` or warns that an account's vote split near
evenly.
